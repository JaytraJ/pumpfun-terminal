...
async function sellManyTokens({ wallets, mint, amountTokensPerWallet = null, percentPerWallet = null, perWalletPercentMap = null, slippagePercent = DEFAULT_SLIPPAGE_PERCENT, priorityFeeSol = DEFAULT_PRIORITY_FEE_SOL, pool = DEFAULT_POOL, concurrency = 4, retries = 2, sequential = false, onProgress = null }) {
  ...
  async function getTokenRawBalance(owner) {
    ...
    for (const it of res.value) {
      ...
      const amt = String(info?.tokenAmount?.amount || '0');
      try {
        total += BigInt(amt);
      } catch {}
    }
    ...
    const runNext = async () => {
      ...
      while (active < limit && queue.length) {
        ...
        (async () => {
          const pub = item.publicKey;
          try {
            let tokensRaw = amountTokensPerWallet != null ? toTokenRawFromUi(amountTokensPerWallet, decimals) : null;
            ...
            if (tokensRaw == null && thisPct != null) {
              const balRaw = await getTokenRawBalance(pub);
              tokensRaw = percentOfRaw(balRaw, thisPct);
            }
            if (tokensRaw == null && percentPerWallet != null) {
              const balRaw = await getTokenRawBalance(pub);
              tokensRaw = percentOfRaw(balRaw, percentPerWallet);
            }
            if (!tokensRaw || tokensRaw <= 0n) throw new Error('No tokens to sell for this wallet');
            const tokensToSell = tokenUiFromRaw(tokensRaw, decimals);
            ...
            results[thisIndex] = { ok: true, seller: pub, signature: sig, soldTokens: tokensToSell, soldRaw: tokensRaw.toString() };
          } catch (e) {
